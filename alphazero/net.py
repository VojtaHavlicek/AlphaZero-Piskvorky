...
class GomokuNet(nn.Module):
    ...
    def __init__(self, board_size=5):
        ...
        )  # Input channels for Gomoku is 2 (black and white pieces)
        ...
        self.policy_head = nn.Sequential(
            nn.Conv2d(
                num_channels, 2, kernel_size=1
            ),  # Output channels for policy head
            nn.BatchNorm2d(2),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(
                2 * self.board_size * self.board_size, self.board_size * self.board_size
            ),  # Flatten to board size
        )
    ...
    def forward(self, x):
        ...
        return policy, value.squeeze(
            -1
        )  # Squeeze to remove the last dimension for value output
